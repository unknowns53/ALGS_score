...
## 第 4 節 もうひとつの隠れた要因 — 中立死率

本命 4 要因の裏に、似たメカニズムで効くパラメータがもう 1 つあった。リング外死や落下死など、誰のスコアにもならない死の発生率を表す `neutral_death_rate` だ。0.000 → 0.060 で平均試合数 8.94 → 9.19 (Δ +0.25)。これも `scored kills` を 59.1 → 55.5 に減らす経路で大会を伸ばす。「キル証拠の蒸発」という意味では `lost_kill_rate` と同類で、効くメカニズムも同じだ。

![neutral_death_rate の値別の終了試合数分布](../out/plot_equal_sweep_neutral_death_rate.png)
*図 7: neutral_death_rate (誰のスコアにもならない死の発生率) を 0.000 から 0.060 まで動かした分布。図 3 と同じく分布が右に寄っていく。「キル証拠の蒸発」系のパラメータは互いに置き換え可能なほどよく似た反応を示す。*

---

## 第 5 節 順位構造への介入 — placement_kill_sharpness

ここまでは「キル供給量」(lost_kill、neutral_death) と「順位戦としての MP 制」(strength_sigma、mp_win_penalty、respawn) の 2 系統で大会の長さが動くことを見てきた。最後に「キル供給量は変えずに、キルが順位ごとにどれだけ偏って配分されるかだけを動かしたら何が起こるか」という、別軸の介入を試した。

ALGS のキル得点配分は、現実の試合データから推定したテーブルでチームごとに重みづけられる。1 位を取ったチームは平均 7.4 キルを取る一方、16-20 位のチームは平均 0.7 キルにとどまる。つまり 1 位:20 位でキル数の比はおよそ 12:1 で、「上位ほどキルも多く取る」という強い偏りが組み込まれている。

この偏りの強さだけを動かすために、対数空間で勾配のシャープネスをスケールする 1 つのパラメータ `placement_kill_sharpness` を新設した。`sharpness=1.0` で現状の 12:1、`sharpness=0.0` で全順位均等 (キルがチームの実力のみで決まり、順位とは無関係に配分される)、`sharpness=2.0` で 1 位:20 位 ~ 144:1 と極端な上位独占世界になる。
...
平均試合数は 0.0 → 2.0 で 9.70 → 8.50 と Δ -1.21 試合。これは strength_sigma の -1.53 に次ぐ第 2 の大きな効果で、しかも単調に綺麗な反応を返している。注目したいのは `scored kills` (1 試合あたり計上キル総数) がどの sharpness でも 57.2〜57.4 でほぼ不動なことだ。順位構造の偏りは「誰に何キル入るか」を変えるだけで、ロビー全体で稼げるキル得点の総量自体は動かさない。それでも大会の長さがこれだけ動く。

![placement_kill_sharpness の値別の終了試合数分布](../out/plot_equal_sweep_placement_kill_sharpness.png)
*図 8: placement_kill_sharpness を 0.0 (全順位均等) から 2.0 (極端な上位独占) まで動かした分布。キル得点の総量を変えずに「順位ごとの偏り」だけを動かす介入。0.0 (青) で右に大きくシフトし、2.0 (赤) で左に大きくシフトする。*

メカニズムはこう解釈できる。sharpness が高いほど、1 位を取ったチームに大量のキル得点も集中する (順位点 12 + キル点が雪だるま式に増える)。これが繰り返されると、特定チームが急速に 50 点リーチに駆け上がる。逆に sharpness が 0 だと、1 位を取っても順位点 12 しか入らず、キルは順位と無関係に全 20 チームに散ってしまう。雪だるま効果が消え、誰もなかなか 50 点に届かなくなる。

これは第 6 節で扱う漁夫率 (transfer_kill_rate) と同じ「snowball 抑制」のメカニズムだが、効果の大きさが桁違いに違う (transfer_kill_rate=0.20 で Δ +0.08 に対し、PKF を均等化すると Δ +0.64)。順位構造の介入の方が、キルの個別経路 (漁夫) を触るよりも遥かに直接的に snowball を解体する。

ここから言えるのは、「キル系のチューニングは効かない」と短絡してはいけないということだ。MP 圏内チームに対するキルペナルティ (第 3 節) は確かに効かなかったが、それは MP 圏内の決着構造がキルではなく順位で起きるからであって、キル得点配分の偏り自体は大会の長さを大きく動かす要因として確かに存在する。この区別は記事の主張を整理するうえで重要なポイントとなる。

---

## 第 6 節 同じ家族の小さな兄弟 — 漁夫率 (transfer_kill_rate)

第 5 節の PKF と同じ「snowball 抑制」家族にもう 1 つ、効果はずっと小さいがメカニズム的に同型のパラメータがある。ノックしたチームの直後に別チームが横取りして得点を奪う確率を表す `transfer_kill_rate` だ。試合の最終的なキル得点総量は変わらない (誰かが得点する) ので、平均試合数は動かないだろう、と予想していた。
...
漁夫が多いと、ノックを取ったチームに得点が集中せず、横取りで分散するので、特定チームへの雪だるま的な加点が抑制される。結果として 50 点リーチが平均的に遅れる、というメカニズム。第 5 節 PKF と同じ snowball 抑制で、効果の方向も一致する。違いは規模で、漁夫率は「すでに発生したキルのうち何 % が横取りされるか」というかなり局所的な介入なのに対し、PKF は「順位ごとの配分構造そのもの」というロビー全体の介入なので、ここまで差がつく。
...
| 図 7 | `../out/plot_equal_sweep_neutral_death_rate.png` | 第 4 節 | 中立死率の効き (図 3 と相似) |
| 図 8 | `../out/plot_equal_sweep_placement_kill_sharpness.png` | 第 5 節 | 順位構造への介入 |
| 図 9 | `../out/plot_equal_sweep_transfer_kill_rate.png` | 第 6 節 | 漁夫率の意外な効き |

note 投稿時は各 PNG を個別アップロードして該当箇所に差し替えること。

未採用の PNG (`win_beta`, `placement_win_correlation`, `base_match_noise`, `volatility_mean`, `rank_beta`, `kill_beta`, `respawn_dispersion`, `mp_pressure_lost_kill_multiplier`, `chaos_multiplier`, `revive_knock_mean`) はすべて第 7 節「効かなかった 7 個」もしくは「効かない確認」に該当し、図にすると 5 本がほぼ完全に重なるだけなので埋め込みは見送り。`chaos_multiplier` は `lost_kill_rate` と同メカニズム (記事改訂前は第 4 節で独立して扱っていたが冗長と判断して削除し、現第 5 節 `placement_kill_sharpness` に差し替え)。`revive_knock_mean` は `MatchResult.revived_knocks` に記録されるだけで `allocate_kills()` / `scored_kills` に渡らないため、構造的に大会長を動かせない。
...
- 第 9 節末尾でリードの問い (APAC-N が長引きやすい理由) に答えを戻す構成にした
- 数値の検算には `docs/sweep_equal_baseline.md`、過去大会データは `docs/data_validation.md` を参照

### 改訂サイクル 2 で対応した著者コメント
- 「12 試合で終了するルールはない」→ リードの「最大 12 試合」を削除済 (著者編集)、第 1 節表の「12 試合フル開催」を「12 試合を超えて続く確率」に修正、第 9 節等の表現も整理
...
- 「各パラメータの説明がないのでわからない」→ 第 7 節に 7 パラメータの 1 行説明表を追加 (parameters.md からの引用)

### 改訂サイクル 3 (PKF 追加・chaos 削除)
- 「本質的でないパラメータがいくつかあるので、代わりに PLACEMENT_KILL_FACTOR の検討を」→ 第 4 節から chaos_multiplier の段落を削除 (lost_kill_rate と同メカニズムで冗長)、新第 5 節「順位構造への介入 — placement_kill_sharpness」を新設。これに伴い PLACEMENT_KILL_FACTOR を `SimulationConfig.placement_kill_sharpness` として外出し (log 空間スケーリング、`sharpness=1.0` で既存タプルそのまま)。
- 「revive_knock_mean の検証も」→ コード読みでは `MatchResult.revived_knocks` 専用フィールドでスコアに無関与のはずだが、実測スイープも回した。本文には載せず `docs/sweep_equal_baseline.md` に表だけ追加 (本記事第 9 節まとめ第 3 階層に 1 項目追加で言及済み)。
